...
class Blockchain {
    ...
    calculateTotalDifficulty(chain) {
        return chain.reduce((sum, block) => {
            return sum + Math.pow(2, block.difficulty || 0);
        }, 0);
    }

    findForkPoint(newChain) {
        let forkPoint = 0;
        while (forkPoint < this.blocks.length && 
               forkPoint < newChain.length && 
               this.blocks[forkPoint].hash === newChain[forkPoint].hash) {
            forkPoint++;
        }
        return forkPoint;
    }
    ...
    replaceChain(newBlockchain) {
        ...
        if (!Array.isArray(newBlockchain)) {
            throw new BlockchainAssertionError('Invalid chain format');
        }
        ...
        try {
            ...
            const currentDifficulty = this.calculateTotalDifficulty(this.blocks);
            const newDifficulty = this.calculateTotalDifficulty(newBlockchain);
            
            if (newDifficulty <= currentDifficulty) {
                console.error('New chain has less or equal difficulty than current chain');
                throw new BlockchainAssertionError('New chain has less or equal difficulty');
            }

            const forkPoint = this.findForkPoint(newBlockchain);
            console.info(`Fork point found at index ${forkPoint}`);

            const newBlocks = newBlockchain.slice(forkPoint);
            const revertedBlocks = this.blocks.slice(forkPoint);

            revertedBlocks.forEach(block => {
                block.transactions.forEach(tx => {
                    if (tx.type !== 'reward' && tx.type !== 'fee') {
                        this.transactions.push(tx);
                    }
                });
            });

            this.blocks = this.blocks.slice(0, forkPoint).concat(newBlocks);
            this.blocksDb.write(this.blocks);

            newBlocks.forEach(this.removeBlockTransactionsFromTransactions.bind(this));

            console.info('Chain replaced successfully');
            this.emitter.emit('blockchainReplaced', newBlocks);

        } catch (ex) {
            console.error('Error replacing chain:', ex);
            throw new BlockchainAssertionError('Invalid chain for replacement', null, ex);
        }
    }
